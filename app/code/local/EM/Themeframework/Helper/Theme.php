...
class EM_Themeframework_Helper_Theme {
	...
	function display($root, $layout = '1column') {
		...
		if ($isPreview) {
			...
			/* Get current package theme */
			$storeId = Mage::app()->getStore()->getId();
			$packageTheme = Mage::getStoreConfig('design/package/name',$storeId).'/'. Mage::getStoreConfig('design/theme/default',$storeId);
			$collection = Mage::getModel('themeframework/area')->getCollection()
							->addStoreFilter($storeId)
							->addFilter('layout', array('eq' => $layout))
							->addFilter('package_theme', array('eq' => $packageTheme))
